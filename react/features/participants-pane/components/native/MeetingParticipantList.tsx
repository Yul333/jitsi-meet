...
class MeetingParticipantList extends PureComponent<IProps> {
	...
	_renderParticipant({ item /* , index, separators */ }: any) {
		...
		const participant =
			item === _localParticipant?.id
				? _localParticipant
				: _remoteParticipants.get(item)
	...
	_openContextMenu() {
		this.props.dispatch(
			openSheet(BreakoutRoomContextMenu, {
	...
	render() {
		...
		const containerStyleModerator = onlyMeetingParticipants
			? styles.meetingListFullContainer
			: styles.meetingListContainer
		const containerStyle = isLocalModerator
			? containerStyleModerator
			: styles.notLocalModeratorContainer
		const finalContainerStyle =
			_participantsCount > 6 ? containerStyle : undefined
		const { color, shareDialogVisible } = _inviteOthersControl
		const _visitorsLabelText =
			_visitorsCount > 0
				? t("participantsPane.headings.visitors", { count: _visitorsCount })
				: undefined
		const onLongPress = _isCurrentRoomRenamable
		...
		return (
			<>
				{_visitorsCount > 0 && (
					<Text style={styles.visitorsLabel}>{_visitorsLabelText}</Text>
				)}
				<CollapsibleList
